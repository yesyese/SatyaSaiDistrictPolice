...
const AuditLogTableRow = ({ log }) => {
  let auditStatusText = 'N/A';
  let auditStatusBgColor = 'bg-gray-700';
  let auditStatusTextColor = 'text-gray-100';

  if (log.action && log.action.includes('LOGIN') && log.details && log.details.includes('successfully')) {
    auditStatusText = 'Success';
    auditStatusBgColor = 'bg-green-800';
  } else if (log.action && log.action.includes('Failed')) {
    auditStatusText = 'Failure';
    auditStatusBgColor = 'bg-red-800';
  } else if (log.status) {
    auditStatusText = log.status;
    switch (log.status) {
      case 'Success':
        auditStatusBgColor = 'bg-green-800';
        break;
      case 'Failure':
        ...
        break;
      case 'Pending':
        auditStatusBgColor = 'bg-yellow-800';
        break;
      default:
        auditStatusBgColor = 'bg-gray-700';
    }
  } else {
    auditStatusText = 'N/A';
    auditStatusBgColor = 'bg-gray-700';
  ...
  return (
    <tr className="border-b border-gray-700 hover:bg-gray-700 transition-colors duration-200">
      ...
      <td className="px-6 py-4 text-left">
        <span className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold ${auditStatusBgColor} ${auditStatusTextColor}`}>
          {auditStatusText}
        </span>
      </td>
    </tr>
...
function SecurityPage() {
  ...
  const fetchAuditLogs = async (currentFilters, isInitialLoad = false) => {
    ...
    try {
      ...
      if (currentFilters.status !== 'All') apiFilters.status = currentFilters.status;
    ...
    <>
              ...
              <tbody>
                {auditLogsLoading ? (
                  <tr className="text-center">
                    <LoadingTableRow colSpan={5} message="Loading audit logs..." />
                  </tr>
                ) : auditLogs.length > 0 ? (
                  auditLogs.map(log => (
                  ...
                  <tr className="text-center">
                    <td colSpan="5" className="py-4 text-gray-400">No audit logs found.</td>
                  </tr>
                )}
              </tbody>
            </table>
      ...
      <style>{`
          ...
          input[type="date"] {
              ...
              -moz-appearance: none;
              appearance: none;
              background-image: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="%239CA3AF" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucude-calendar"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>');
              background-repeat: no-repeat;
              background-position: right 0.75rem center;
              background-size: 1rem;
          ...
          input[type="date"]::-webkit-calendar-picker-indicator { opacity: 0; } /* WHAT CHANGED: KEEP THIS AT 0 FOR CUSTOM ICON */
      `}</style>
    </>
  );
}
