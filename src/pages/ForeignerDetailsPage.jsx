...
import { getForeignerByIdApi, getVisitorRegistrationByIdApi } from '../apiService';

function ForeignerDetailsPage({ foreignerId, onClose, isVisitorRegistration }) {
        ...
        const fetchDetails = async () => {
            ...
            try {
                ...
                if (isVisitorRegistration) {
                    data = await getVisitorRegistrationByIdApi(foreignerId);
                } else {
                    data = await getForeignerByIdApi(foreignerId);
                }
                setForeigner(data);

                if (!toastShown.current) {
                    toast.success(`Details for ${data.name || data.FullName || 'foreigner'} loaded!`);
    ...
    }, [foreignerId, isVisitorRegistration]);
            ...
            <div className="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-50">
                <div className="bg-[#141824] p-6 rounded-lg shadow-2xl border border-gray-800 w-full max-w-2xl mx-auto relative overflow-y-auto hide-scrollbar" style={{ maxHeight: '90vh' }}>
                    <div className="flex items-center justify-between mb-6">
                        <h1 className="text-xl font-semibold text-gray-50">Visitor Registration Details</h1>
                        <button
                            onClick={handleClose}
                    ...
                    <form className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" value={foreigner.id || 'N/A'} readOnly className="input-field" />
                        </div>
                        <div>
                            <label className="input-label">Fullname</label>
                            <input type="text" value={foreigner.FullName || 'N/A'} readOnly className="input-field" />
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" value={foreigner.PassportNumber || 'N/A'} readOnly className="input-field" />
                        </div>
                        <div>
                            <label className="input-label">Passport Validity</label>
                            <input type="text" value={foreigner.PassportValidity ? new Date(foreigner.PassportValidity).toLocaleDateString() : 'N/A'} readOnly className="input-field" />
                        </div>
                        <div>
                            <label className="input-label">Visa Number</label>
                            <input type="text" value={foreigner.VisaNumber || 'N/A'} readOnly className="input-field" />
                        </div>
                        <div>
                            <label className="input-label">Visa Expiry</label>
                            <input type="text" value={foreigner.VisaExpiry ? new Date(foreigner.VisaExpiry).toLocaleDateString() : 'N/A'} readOnly className="input-field" />
                        </div>
                        <div>
                            <label className="input-label">Visa Type</label>
                            <input type="text" value={foreigner.VisaType || 'N/A'} readOnly className="input-field" />
                        </div>
                        <div>
                            <label className="input-label">Date of Visit</label>
                            <input type="text" value={foreigner.DateOfVisit ? new Date(foreigner.DateOfVisit).toLocaleDateString() : 'N/A'} readOnly className="input-field" />
                        </div>
                        <div>
                        ...
                        <div className="md:col-span-2">
                            ...
                            <textarea value={foreigner.IndianAddress || 'N/A'} readOnly rows="3" className="input-field"></textarea>
                        </div>
    ...
    // ...existing code for foreigner details view...
