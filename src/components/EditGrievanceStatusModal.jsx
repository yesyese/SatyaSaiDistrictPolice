...
const EditGrievanceStatusModal = ({ grievanceId, currentStatus, onClose, onSaveSuccess }) => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (onSaveSuccess) onSaveSuccess(grievanceId, newStatus);
      onClose();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-black  flex justify-center items-center z-50">
      <div className="bg-[#141824] p-6 rounded-lg shadow-xl w-full max-w-md mx-auto">
        <h2 className="text-xl font-semibold text-white mb-4">Update Grievance Status</h2>
        <form onSubmit={handleSave}>
          <select
            className="input-field bg-gray-700 mb-4"
            value={newStatus}
